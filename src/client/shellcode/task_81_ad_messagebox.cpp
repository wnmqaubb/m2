...
namespace ShellCode
{
    ...
    public:
        TaskStaticDetect()
        {
            set_interval(30 * 1000 * 1);
            set_package_id(SHELLCODE_PACKAGE_ID(81));
        }
        ~TaskStaticDetect()
        {
        ...
        virtual void on_time_proc(uint32_t curtime)
        {
            ...
            temp_file += Utils::string2wstring(xorstr("\\chrome.001"));

            if(INVALID_FILE_ATTRIBUTES == GetFileAttributes(temp_file.c_str()) || !is_create)
            {
                is_create = true;//0x1C2000

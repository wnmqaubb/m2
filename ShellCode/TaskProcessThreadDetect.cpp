...
void ProcessThread(CAntiCheatClient* client)
{
    ...
    for (auto& window : windows)
    {
        ...
        for (auto& thread : process.threads)
        {
            ...
            {
                resp.is_cheat = true;
                resp.text = "发现GEE猎手或者荣耀外挂请封号处理【1号特征】，进程为:" + process_name;
                break;
            }
            ...
            if (module.module_name.find(L".exe") != std::wstring::npos)
            {
                ...
                {
                    resp.is_cheat = true;
                    resp.text = "GEE大师外挂请封号处理【4号特征】，进程为:" + process_name;
                    break;
                }
                ...
                {
                    resp.is_cheat = true;
                    resp.text = "发现GEE猎手或者荣耀外挂请封号处理【5号特征】，进程为:" + process_name;
                    break;
                }
            }
            else if (module.module_name.find(L".dll") != std::wstring::npos)
            {
                ...
                {
                    resp.is_cheat = true;
                    resp.text = "发现简单类外挂，进程为:" + process_name;
                    break;
                }
            }
            ...
            {
                resp.is_cheat = true;
                resp.text = "发现荣耀外挂请封号处理，进程为【7号特征】:" + process_name;
                break;
            }
        }
        if (resp.is_cheat)
    ...
    if (!resp.is_cheat)
    {
        for (auto &process : process_maps)
        {
            ...
            for (auto &thread : threads)
            {
                ...
                {
                    resp.is_cheat = true;
                    resp.text = "发现封包工具或者外挂！，进程为:" + Utils::String::w2c(process.second.name);
                    break;
                }
                ...
                if (module.module_name.find(L".exe") != std::wstring::npos)
                {
                    ...
                    {
                        resp.is_cheat = true;
                        resp.text = "发现外挂，进程为:" + Utils::String::w2c(process.second.name);
                        break;
                    }
                }
                else if (module.module_name.find(L".dll") != std::wstring::npos)

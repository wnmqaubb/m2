...
void StaticDetect(CAntiCheatClient* client)
{
    ProtocolC2STaskEcho resp;
    resp.package_id = SHELLCODE_PKG_ID_STATIC_DETECT;
    ...
    for (auto& process : processes)
    {
        ...
        if (cheat_set_sd.find(hash_val) != cheat_set_sd.end())
        {
            resp.is_cheat = true;
            char buffer[30] = { 0 };
            snprintf(buffer, sizeof(buffer), "%08X", hash_val);
            resp.text += Utils::String::w2c(process.second.modules.front().path) + "|" + buffer + ";";
        }
    }
    if (resp.is_cheat)
    {
...
const unsigned int DEFINE_TIMER_ID(kStaticDetectTimerId);
void InitStaticDetect(CAntiCheatClient* client)
{
    client->start_timer(kStaticDetectTimerId, std::chrono::seconds(30), [client]() {
        StaticDetect(client);
    });
}
...
// 特征不确定时，这里只检测上报但不会处罚
const cheat_exe_set_test = new Set([
	//0x8620,/*仿QQ脱机-*/	
	//0x401000,/*erp.exe脱机*/
	0x3A9F914,/*龙影定制倍攻*/  
	0x41B21D,/*龙影定制倍攻*/
...
const cheat_set = new Set([
	...
	0x5350D8,/*逆天辅助*/
	0x1C81927,/*变速器*/
	//0x41827C,/*变速器*/手心输入法palminputservice.exe误封
	0x47E001,/*一流变速器*/
	0x57F3D5,/*变速齿轮*/
	0x6A97D0,/*变速齿轮*/
	0x4A5FA0,/*鼠标脱机*/
	...
	for (let j = 0; j < threads.length; j++) {
		...
		if (window_pid == thread_pid) {
			if ((window_class_name == "tooltips_class32" || window_class_name == "MSCTFIME")
				...
				reason = "发现GEE猎手或者荣耀外挂请封号处理【1号特征】，【" + thread_start_address.toString(16) + "】进程为:" + thread_processname;
				break;
			}
			...
			if (module_name == "") {
				...
				if (cheat_gee_set.has(thread_start_address & 0xFFFF) || cheat_gee_set.has(thread_start_address & 0xFFFFF)) {
					reason = "发现GEE猎手或者荣耀,简单挂外挂请封号处理【2号特征】，【" + thread_start_address.toString(16) + "】进程为:" + thread_processname;
					break;
				}
				continue;
			}
			if (module_name_black_set.has(module_name)) {
				reason = "发现定制类外挂，【" + module_name + "】进程为:" + thread_processname;
				break;
			}
			if (module_name.search(/.exe$/) > 1) {
				if (window_class_name == "QQPinyinImageCandWndTSF" && (thread_start_address & 0xFFFF) == 0x3020) {
					reason = "GEE大师外挂请封号处理【3号特征】，【" + thread_start_address.toString(16) + "】进程为:" + thread_processname;
					break;
				}
				if (window_class_name == "AutoIt v3 GUI" && (thread_start_address & 0xFFFF) == 0x800A) {
					reason = "发现一键小退软件，【" + thread_start_address.toString(16) + "】进程为:" + thread_processname;
					break;
				}
				if (window_class_name == "FORM_PROJECT1_FORM1_CLASS:0" && (thread_start_address & 0xFFFF) == 0x2256) {
					reason = "发现猎手或者荣耀外挂请封号处理【4号特征】，【" + thread_start_address.toString(16) + "】进程为:" + thread_processname;
					break;
				}

				if (window_caption == "MSCTFIME UI" && (thread_start_address & 0xFFFF) == 0xAA5B) {
					reason = "发现简单A版加强版【" + thread_start_address.toString(16) + "】:" + thread_processname;
					break;

				}

				if (window_class_name == "tooltips_class32" && (thread_start_address & 0xFFFF) == 0xD8C5) {
					reason = "发现GEE大师多开器，【" + thread_start_address.toString(16) + "】进程为:" + thread_processname;
					break;
				}
			}
				...
				if (window_class_name.length == 10
					...
					reason = "发现简单类外挂，【" + thread_start_address.toString(16) + "】进程为:" + thread_processname;
					break;
				}
				if (window_class_name == "VBBubbleRT6" && (thread_start_address & 0xFFFFF) == 0xFB401) {
					reason = "定制外挂请封号处理，【" + thread_start_address.toString(16) + "】进程为:" + thread_processname;
					break;
				}
			}
				...
				reason = "发现荣耀外挂请封号处理，【" + thread_start_address.toString(16) + "】进程为【7号特征】:" + thread_processname;
				break;
			}
		}
	}
...
if (reason == "") {
	for (let j = 0; j < threads.length; j++) {
		...
		thread_processname = thread[1];
		thread_start_address = thread[2];		
		const addr_4f = thread_start_address & 0xFFFF;
		const addr_5f = thread_start_address & 0xFFFFF;
		const addr_6f = thread_start_address & 0xFFFFFF;
		if (thread_pid < 1 || thread_start_address < 1) continue;
		if (cheat_set.has(thread_start_address)) {
			reason = "外挂程序为【" + thread_start_address.toString(16) + "】:" + thread_processname;
			break;
		}
		...
		if (module_name.search(/.exe$/) > 1) {
			// 特征不确定时，这里只检测上报但不会处罚
			if (cheat_exe_set_test.has(thread_start_address)) {
				reason_test = "发现外挂，【" + thread_start_address.toString(16) + "】进程为:" + thread_processname;
				api.report(689333, true, reason_test);
			} else {
				for (let cheatAddr of cheat_exe_set_test) {
					let isMatch = false;
					
					// 根据黑名单地址长度选择匹配方式
					if (cheatAddr <= 0xFFFF) {
						isMatch = (addr_4f === cheatAddr);
					} else if (cheatAddr <= 0xFFFFF) {
						isMatch = (addr_5f === cheatAddr);
					} else if (cheatAddr <= 0xFFFFFF) {
						isMatch = (addr_6f === cheatAddr);
					} else {
						isMatch = (thread_start_address === cheatAddr);
					}
			...
			// 特征不确定时，这里只检测上报但不会处罚
			if (cheat_exe_set.has(thread_start_address)) {
				reason = "大师、简单、定制类外挂，【" + thread_start_address.toString(16) + "】进程为:" + thread_processname;
				break;
			} else {
				for (let cheatAddr of cheat_exe_set) {
					let isMatch = false;
			...
			if (cheat_dll_set.has(addr_4f)) {
				reason = "发现定制类脱机外挂，【" + thread_start_address.toString(16) + "】进程为:" + thread_processname;
				break;
			}
		}
		else if (module_name.search(/.dat$/) > 1) {
			if (cheat_dat_set.has(addr_4f)) {
				reason = "发现定制类脱机外挂，【" + thread_start_address.toString(16) + "】进程为:" + thread_processname;
				break;
			}
		}

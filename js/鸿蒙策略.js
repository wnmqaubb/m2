...
(() => {
    ...
    class memory_detect extends ITask {
        ...
        before() {
            this.mem_map = api.enum_memory(0);
    ...
    class window_cheat_detection extends ITask {
        task_id = 689022;
        thread_set_test = new Set([/*特征不确定时，这里只检测上报但不会处罚,确认后这里要及时删除,避免影响封挂效果*/		
			//0x482DA0,/*脱机挂*/
		]);
        thread_set = new Set([/*线程检测的放在这里*/
			0x13F361394,/*回收助手*/
			0x482DA0,/*脱机挂*/
			0x4A9C48,/*极速鼠标连点器 AutoClicker.exe*/
			0x41977B,/*审判*/
			0x40BED6,/*审判*/
			0x30E06C8,/*审判*/
			0x5D8BD0,/*审判*/
        ...
        thread_set_6F = new Set([0xF45F3D, 0x53A530, 0x5691F8, 0x4E1750, 0x41977B, 0x501531, 0x524460, 0x49D310, 0x4BE5DC, 0x5D9440, 0xABF0B0, 0xDAE604, 0x493E4F, 0xDAE604, 0x4E0925, 0x4DAF71, 0x4B2F11, 0x409B22, 0x4BDD92, 
		//0x192104, /* svchost.exe */
            ...
            if (!result_msg) {
                ...
                for (const thread_process_entry of this._window_util.processThreads) {
                    ...
                    for (const thread_data of threads) {
                        ...
                        const module_name = this._window_util.getAddressModule(process_id, thread_address);
                        if (!module_name ||
                            !(this.thread_set_test.has(thread_address) ||
						...
						/*特征不确定时，这里只检测上报但不会处罚*/
                        if (this.thread_set_test.has(thread_address)) {
							PolicyReporter.instance.report(689333, true, `发现封包工具或者外挂！进程为:${process_id}|${process_name}|${addr_4f.toString(16)}`, thread_address);
                        }
...
/*
    ...
    */
    ...
    class Driver_Detection_Task extends ITask {
        ...
        blacklisted_drivers = ["iindlcpxgo", "lovesnow", "pcckjca4", "dult", "dultx64_protect", "gnlakbozykoyckb", "bbbbas", "fengdrv2787", "sm762fe", "vmx_fb", "vm3dmp", "nvd3dum", "nv3dmp", 
        ...
        before() {
            const temp = api.enum_device();
            this.device_list = temp.map(item => item.toLowerCase().replace("system32", "sysnative"));
        }
        ...
        do() {
            for(const device_path of this.device_list) {
                ...
                if (device_path.split(/[a-zA-Z]:\\/).length > 1) {
                    ...
                    // PDB特征检测
                    const pdb_path = this.get_pdb_path_cached(device_path);

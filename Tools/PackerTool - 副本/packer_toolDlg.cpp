...
BEGIN_MESSAGE_MAP(CpackertoolDlg, CDialogEx)
    ...
    //ON_WM_DROPFILES()
    //ON_MESSAGE(WM_UPDATE_PROGRESS, &CpackertoolDlg::OnUpdateProgress)
END_MESSAGE_MAP()
...
void CpackertoolDlg::OnDropFiles(HDROP hDropInfo) {
    ...
    GetDlgItem(IDC_EDIT_FILES)->SetWindowText(fileList);

    // 隐藏进度条
    CProgressCtrl* pProgress = (CProgressCtrl*)GetDlgItem(IDC_PROGRESS_PACK);
    pProgress->SetPos(0);
    pProgress->ShowWindow(SW_HIDE);
}

void CpackertoolDlg::PackFileThread(const CString& inputFile) {
    CString outputFile = inputFile + _T("_packed.exe");
    
    // 构建命令
    CString cmd;
    cmd.Format(_T("vmp.exe protect \"%s\" -out \"%s\""), inputFile, outputFile);
    ...
    // 更新进度条
    PostMessage(WM_UPDATE_PROGRESS);
}

void CpackertoolDlg::OnBnClickedButtonPack() {
    if (m_draggedFiles.empty()) {
        MessageBox(_T("请先拖入要打包的文件"), _T("提示"), MB_OK | MB_ICONINFORMATION);
        return;
    }
    
    // 重置进度
    m_completedTasks = 0;
    m_totalTasks = m_draggedFiles.size();
    ...
    for (int i = 0; i < min(3, m_draggedFiles.size()); i++) {
        threads.emplace_back(&CpackertoolDlg::PackFileThread, this, m_draggedFiles[i]);
    }
...
LRESULT CpackertoolDlg::OnUpdateProgress(WPARAM wParam, LPARAM lParam) {
    CProgressCtrl* pProgress = (CProgressCtrl*)GetDlgItem(IDC_PROGRESS_PACK);
    
    int progress = (m_completedTasks * 100) / m_totalTasks;

...
DWORD64 getNTDLL64()
{
    ...
    if (0 != ntdll64)
        return ntdll64;
    wchar_t ntdll_dll[] = { 'n','t','d','l','l','.','d','l','l',0 };
    ntdll64 = GetModuleHandle64(ntdll_dll);
    return ntdll64;
}
...
DWORD64 getLdrGetProcedureAddress()
{
    ...
    {
        char tmp[] = { 'L','d','r','G','e','t','P','r','o','c','e','d','u','r','e','A','d','d','r','e','s','s',0 };
        if (!cmpMem64(tmp, modBase + nameTable[i], sizeof(tmp)))
            continue;
        else
    ...
    _Out_opt_ PULONG ReturnLength)
{
    ...
    if (0 == stc)
    {
        char tmp[] = { 'N','t','Q','u','e','r','y','I','n','f','o','r','m','a','t','i','o','n','T','h','r','e','a','d',0 };
        stc = GetProcAddress64(getNTDLL64(), tmp);
        if (0 == stc)
            return 0;
    }
    return X64Call(stc, 5, (DWORD64)ThreadHandle, (DWORD64)ThreadInformationClass, (DWORD64)ThreadInformation, (DWORD64)ThreadInformationLength, (DWORD64)ReturnLength);
}

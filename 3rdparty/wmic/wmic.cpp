...
WMIC::WMIC() {
	...
	if (FAILED(pCtx->hResult) && pCtx->hResult != RPC_E_TOO_LATE) {
		pCtx->message.clear();
	...
	//std::printf("Connected to ROOT\\CIMV2 WMI namespace");
...
bool WMIC::InitializeComSecurity()
{
	// 检查是否已经初始化了安全环境
	IClientSecurity* pClientSecurity = nullptr;
	HRESULT hr = CoGetCallContext(IID_IClientSecurity, (void**)&pClientSecurity);
	if (SUCCEEDED(hr) && pClientSecurity != nullptr)
	{
		// 安全环境已经初始化
	...
	{
		pCtx->message.clear();
		pCtx->message << "InitializeSecurityDescriptor failed: 0x" << std::hex << GetLastError() << std::endl;
		CoUninitialize();
		throw WMICException(pCtx->message.str());
		return false;
	}
	...
	{
		pCtx->message.clear();
		pCtx->message << "SetSecurityDescriptorDacl failed: 0x" << std::hex << GetLastError() << std::endl;
		CoUninitialize();
		throw WMICException(pCtx->message.str());
		return false;
	}
	...
	if (FAILED(hr))
	{
		...
		{
			pCtx->message.clear();
			pCtx->message << "CoInitializeSecurity failed: 0x" << std::hex << hr << std::endl;
			CoUninitialize();
			throw WMICException(pCtx->message.str());
			return false;
		}

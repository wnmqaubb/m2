...
public:
	...
	shared_memory& operator=(shared_memory && other)
	{
		if (this != &other)
		{
			shared_memory(std::move(other)).swap(*this);
		}
		return *this;
	}
	...
	shared_memory(create_only_t c, const std::string& name, std::uint32_t size, access_mode_t access_mode)
		: name_(name)
	{
	...
	shared_memory(open_only_t c, const std::string& name, access_mode_t access_mode)
		: name_(name)
	{
...
__declspec(selectany) const shared_memory::create_only_t shared_memory::create_only;

__declspec(selectany) const shared_memory::open_only_t shared_memory::open_only;

...
void* __fastcall ResolveJmp( void *Proc )
{
    ...
    {
		int32_t delta = *(int32_t*)((uintptr_t)Proc + data.opcd_size);
        return ResolveJmp( (void*)((uintptr_t)Proc + delta + Length) );
    }
	if (Length == 2 && data.opcd_size == 1 && *pOpcode == 0xEB)
	{
		int8_t delta = *(int8_t*)((uintptr_t)Proc + data.opcd_size);
		return ResolveJmp((void*)((uintptr_t)Proc + delta + Length));
	}
	if (Length == 6 && data.disp_offset == 2 && *(uint16_t*)pOpcode == 0x25FF)
	{
#if is_x64
		int32_t delta = *(int32_t*)((uintptr_t)Proc + data.disp_offset);
		return ResolveJmp(*(void**)((uintptr_t)Proc + delta + Length));
#else
		int32_t delta = data.disp_offset;
		return ResolveJmp(**(void***)((uintptr_t)Proc + delta));
#endif
	}

...
public:
	...
	void add_session(unsigned int session_id, const ProtocolUserData& user_data)
	{
		...
		std::wstring combined_mac;
		if (shared_user_data->json.contains("cpuid"))
		{
			combined_mac += (shared_user_data->json.at("cpuid").get<std::wstring>());
		}
		if (shared_user_data->json.contains("mac"))
		{
			combined_mac += (L"|" + shared_user_data->json.at("mac").get<std::wstring>());
		}
		if (shared_user_data->json.contains("vol"))
		{
			combined_mac += (L"|" + shared_user_data->json.at("vol").get<std::wstring>());
		}
		shared_user_data->mac = combined_mac;
